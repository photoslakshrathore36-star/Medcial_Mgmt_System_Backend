...
router.post('/orders', auth, adminOnly, async (req, res) => {
  try {
    ...
    const [r] = await db.query(
      ...
      [finalOrderNo, name, (category_id !== '' && category_id != null) ? category_id : null, client_name, client_phone, description, priority || 'medium', safeOrderDate, safeDeadline, total_amount || 0, notes, req.user.id, orgId]
    );
...
router.put('/orders/:id', auth, adminOnly, async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      ...
      [name, (category_id !== '' && category_id != null) ? category_id : null, client_name, client_phone, description, status, priority, safeOrderDate, safeDeadline, total_amount, notes, req.params.id]
    );
